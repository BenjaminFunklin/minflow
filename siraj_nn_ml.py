...
for j in range(60000):
    ...
    if(j % 1000) == 0:   # Only print the error every 10000 steps, to save time and limit the amount of output.
        ...
        error_a.append(abs(l2_error[0][0]))
        print("l2: " + str(np.mean(np.abs(l2))))
        weights_a.append(l2[0][0])
        weights_b.append(l2[1][0])
        weights_c.append(l2[2][0])
        # np.hstack(weights_a,l2[0])
        pdb.set_trace()
...
# n = np.array(1, len(error),1)
n = np.arange(len(error))
# plt.plot(n, error)
plt.plot(weights_a, error_a)
...
# weights_a, weights_b = np.meshgrid(weights_a, weights_b)

# # Plot the surface.
# surf = ax.plot_surface(weights_a, weights_b, error, cmap=cm.coolwarm,
