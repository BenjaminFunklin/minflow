...
class Add(Node):
    ...
    def forward(self):
        ...
        """
        l = len(self.inbound_nodes)
        nodeLen = range(0,l)
        total = 0
        for n in nodeLen:
            total+=self.inbound_nodes[n].value
            print ("here I am all alone", self.inbound_nodes[n].value,self.inbound_nodes[n])
...
def topological_sort(feed_dict):
    ...
    for m in input_nodes:
        print (m.value,"input_node value")
    
    #we create a new dictionary G
    G = {}
    nodes = [n for n in input_nodes]
    for u in nodes:
        print (u.outbound_nodes[0].value,"node value")
    print (nodes, "nodes")
    while len(nodes) > 0:
        n = nodes.pop(0)
        if n not in G:
